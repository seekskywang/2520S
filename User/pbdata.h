...
extern u32 scan_V[130],scan_I[430];
...
typedef struct //多路交流电阻显示寄存器
{
    Test_ValueTypedef Disp_Res[10];
    Test_ValueTypedef Disp_V[10];
    u8 Res_state[10];
    u8 V_State[10];

}jk510_Disp_Typedef;
extern jk510_Disp_Typedef jk510_Disp;
typedef struct
{
    u8 trig;
    u8 speed;
...
typedef struct 
{
    ...
    u8 comp;
    u8 outmode;
    Sort_TypeDef Res_Hi[10];
    Sort_TypeDef Res_Lo[10];
...
typedef struct 
{
    ...
    u8 comp;
    //u8 outmode;
    Sort_TypeDef V_Hi[10];
    Sort_TypeDef V_Lo[10];


}V_comp_Typedef;
typedef struct
{
	jk510_SSet_Typedef jk510_SSet;//4个参数设置
...
//    u8 Comp_mode;
//    u8 comp_Out;
//    SystemRes_TypeDef Only_Hi_limit;
//    SystemRes_TypeDef Only_Lo_limit;
//    SystemRes_TypeDef Hi_limit[10];
//    SystemRes_TypeDef LO_limit[10];

//}Comp_Date_Typedef;
//typedef struct
//{
//    SetDate_Typedef SetDate;//设置
//    Comp_Date_Typedef Comp_Date;//比较数据
//    Debug_Value_Typedef Debug_Value[DEBUG_RANGE];//校正数据
//	Sys_Setvalue_Typedef Sys_Setvalue;//系统设置
////	float clear;
////	float V_Clear;
//    float Clear[10][RANGE_MAX+1];
//     float Clear_V[10][2];
//    vu8 fac_num[10];
//    
...
//typedef struct
//{
//    SystemRes_TypeDef Disp_R[10];
//    SystemRes_TypeDef Disp_V[10];
//    u8 R_State[10];
//    u8 V_State[10];


//}JK510_Disp_value_Typedef;
